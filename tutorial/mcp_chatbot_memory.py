...
async def run_chatbot():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # We add a checkpointer to persist state between turns
            memory = MemorySaver()
            agent = create_react_agent(model, langchain_tools, checkpointer=memory)

            # Define a unique thread ID for this session
            ...
            while True:
                try:
                    ...
                    with console.status("[bold cyan]Agent is thinking...[/bold cyan]", spinner="aesthetic"):
                        # Pass the config with thread_id to resume previous state
